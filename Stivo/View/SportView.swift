...
struct SportView: View {
    ...
    func saveGoals() {
        if let encoded = try? JSONEncoder().encode(goals) { UserDefaults.standard.set(encoded, forKey: "savedSportGoals") }
    }

    func loadGoals() {
        if let data = UserDefaults.standard.data(forKey: "savedSportGoals"),
