...
struct FinanceView: View {
    ...
    func saveGoals() {
        if let e = try? JSONEncoder().encode(goals) { UserDefaults.standard.set(e, forKey: "savedFinanceGoals") }
    }
    func loadGoals() {
        if let d = UserDefaults.standard.data(forKey: "savedFinanceGoals"),
           let decoded = try? JSONDecoder().decode([Goal].self, from: d) {
            goals = decoded; if !decoded.isEmpty { hasOpenedFinanceBefore = true }
        }
    }
    func syncGoalsToDashboard() { viewModel.financeGoals = goals }
}

#Preview { FinanceView().environmentObject(DashboardViewModel()) }
